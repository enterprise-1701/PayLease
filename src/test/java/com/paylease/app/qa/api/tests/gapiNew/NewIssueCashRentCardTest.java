...
import static com.paylease.app.qa.framework.newApi.newGapi.IssueCashRentCardTransaction.ISSUE_CASH_RENT_CARD_TRANSACTION;
...
import com.paylease.app.qa.testbase.dataproviders.IssueCashRentCardDataProvider;
...
public class NewIssueCashRentCardTest extends ScriptBase {

  private static final String REGION = "gapi";
  private static final String FEATURE = "IssueCashRentCard";

  @Test(dataProvider = "issueCashRentCardXmlInjection", dataProviderClass =
      IssueCashRentCardDataProvider.class)
  public void tcXXXX(String testVariationNum, String payerReferenceId, String payerFirstName,
      String payerLastName, String elementTagName, String expectedValue, String expectedCode,
      String expectedStatus) {
    Logger.info(testVariationNum
        + " Verify that IssueCashRentCard GAPI action is not vulnerable to XML injection");
  ...
  private PayLeaseGatewayRequest buildRequestObject(Credentials credentials,
    ...
    IssueCashRentCardTransaction issueCashRentCardTransaction = new IssueCashRentCardTransaction();

    issueCashRentCardTransaction.setTransactionAction(ISSUE_CASH_RENT_CARD_TRANSACTION);
    issueCashRentCardTransaction.setPayerReferenceId(payerReferenceId);
    issueCashRentCardTransaction.setPayerFirstName(payerFirstName);
    issueCashRentCardTransaction.setPayerLastName(payerLastName);

    transactionList.add(issueCashRentCardTransaction);
